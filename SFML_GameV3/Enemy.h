...
private:
    ...
    sf::Time attackCooldown; // Cooldown between attacks
...
public:
    ...
    void update(sf::RenderWindow& window, float deltaTime) {
        ...
        if (distance != 0) {
            ...
            if (distance <= attackRange && attackClock.getElapsedTime() >= attackCooldown) {
                ...
                attackClock.restart(); // Restart the attack cooldown timer
            }
        }
        if(distance == 0){
            if (distance <= attackRange && attackClock.getElapsedTime() >= attackCooldown) {
    ...
    Enemy& operator=(const Enemy& other) {
        if (this != &other) {
            ...
            // Example: this->health = other.health;
            //Issue was I didn't fill this in because I chatgptd the operator override
